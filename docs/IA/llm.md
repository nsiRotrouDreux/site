...
Alors comment fonctionne ces fameux LLM, eux qui semblent si humains quand on discute avec eux ?
...
!!! tip "La technique "
    ...
    On s'appuie sur trois vecteurs : La requête, la clé , la valeur :
...
2. #### Le fine tuning
  Ici, on va dire que le modèle possède la technique mais ne sait pas l'utiliser : Il va être entrainésur des 'tonnes" d'exemples , il apprend à distinguer les différentes taches : coder, traduire , résumer , répondre aux questions...
...
Existe il des sml spécialisés dans certains domaines  créé à partir de llm ?

!!! success "Un LLM nous explique ce qu'il ne fait pas très bien :"
...
Il existe quantité d'ouvrages sur l'IA, tous présentant le défaut d'être quasi opbsolète avant de sortir . Pour autant, L'intelligence artificielle pour les nuls a l'avantage de sprésenter des notions 'intemporelles' si l'on peut dire de façon claire, concise et précise . C'est déjà beaucoup.
...
_Last but nos least_, ce bon vieux [wikipedia](https://fr.wikipedia.org/wiki/Intelligence_artificielle) n'est jamais boien loin 
